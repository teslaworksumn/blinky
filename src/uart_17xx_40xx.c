...
static CHIP_SYSCON_CLK_T Chip_UART_DetermineClk(LPC_USART_T *pUART) {
	CHIP_SYSCON_CLK_T uartclk;
...
void Chip_UART_Init(LPC_USART_T *pUART)
{
	...
	Chip_SYSCON_PCLKEnable(Chip_UART_DetermineClk(pUART));
...
void Chip_UART_DeInit(LPC_USART_T *pUART)
{
	...
	Chip_SYSCON_PCLKDisable(Chip_UART_DetermineClk(pUART));
}
...
Status Chip_UART_SetBaud(LPC_USART_T *pUART, uint32_t baudrate)
{
	...
	/* Get UART clock rate */
	uClk = Chip_SYSCON_GetPCLKRateFromClk(Chip_UART_DetermineClk(pUART));
